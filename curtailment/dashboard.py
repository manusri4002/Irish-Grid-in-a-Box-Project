...
def generate_grid_data():
    ...
    wind_actual_realized = raw["Wind Actual (MW)"].to_numpy()  # already post-curtailment, per EirGrid's own definition
    net_interconnection = raw["Net Interconnection (MW)"].to_numpy()
    snsp = raw["SNSP (%)"].to_numpy()
    ...
    wind_direction = (225 + rng.uniform(-15, 15, n)) % 360
    pressure = 1002 + 10 * np.cos(np.linspace(0, 2 * np.pi, n))
    temperature = 7 + 4 * np.sin(np.linspace(-np.pi / 2, 1.5 * np.pi, n))
    ...
    curtailment_mwh_estimate = curtailment_mw_estimate * 1.0  # hourly resolution
...
with trend_col:
    with tf.container(border=True):
        ...
        fig_trend.add_trace(go.Scatter(x=df_grid["timestamp"], y=df_grid["demand_mw"], mode="lines", name="System Demand (MW)", line=dict(color=TEXT_COLOR, width=2)))
        fig_trend.add_trace(go.Scatter(x=df_grid["timestamp"], y=df_grid["wind_precurtailment_estimate_mw"], mode="lines", name="Est. Pre-Curtailment Wind (MW)", line=dict(color=ACCENT_BLUE, width=1.5, dash="dash")))
        fig_trend.add_trace(go.Scatter(x=df_grid["timestamp"], y=df_grid["wind_actual_mw"], mode="lines", name="Wind Actual (MW, EirGrid)", line=dict(color=ACCENT_GREEN, width=2.5), fill='tozeroy', fillcolor="rgba(5, 255, 155, 0.04)"))
        fig_trend.add_trace(go.Bar(x=df_grid["timestamp"], y=df_grid["curtailment_mwh_estimate"], name="Est. Curtailment (MWh)", marker_color="rgba(255, 75, 75, 0.4)", yaxis="y"))
...
with price_col:
    with tf.container(border=True):
        fig_price = go.Figure()
        fig_price.add_trace(go.Scatter(x=df_grid["timestamp"], y=df_grid["sem_price_eur"], mode="lines", name="SEM Day-Ahead Price (simulated)", line=dict(color=ACCENT_ORANGE, width=2), fill='tozeroy', fillcolor="rgba(255, 159, 67, 0.04)"))
