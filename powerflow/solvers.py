...
    """Solves nonlinear AC power flow equations using the Newton-Raphson method.

    Formulates power flow mismatch equations and iteratively updates bus voltage
    angles (theta) and magnitudes (V) by solving linear system J * dx = mismatch,
    where J is the polar power flow Jacobian matrix.

    Attributes:
        net (PowerNetwork): The target power system network model.
        max_iter (int): Maximum allowable solver iterations.
        tol (float): Convergence tolerance threshold for power mismatch (in p.u.).
    """
...
        """Executes the Newton-Raphson power flow solution algorithm.

        Iteratively updates voltage magnitudes and angles until active and reactive
        power mismatches fall below the specified tolerance threshold.

        Returns:
            tuple[np.ndarray, np.ndarray]:
                - V (np.ndarray): Final bus voltage magnitudes in per-unit (p.u.).
                - theta (np.ndarray): Final bus voltage phase angles in radians.
...
        # Scheduled active and reactive net power injections: P_net = P_generation - P_load
        P_sched = np.array(
            [b.p_gen - b.p_load for b in self.net.ordered_buses]
        )
        Q_sched = np.array(
            [b.q_gen - b.q_load for b in self.net.ordered_buses]
        )
...
            H = dP / dTheta  (size: n_p x n_p)
            N = dP / dV      (size: n_p x n_q)
            M = dQ / dTheta  (size: n_q x n_p)
            L = dQ / dV      (size: n_q x n_q)
...
        # Initialize Jacobian matrix sub-blocks
        H = np.zeros((n_p, n_p))
        N = np.zeros((n_p, n_q))
        M = np.zeros((n_q, n_p))
        L = np.zeros((n_q, n_q))
