...
Two gradient-boosted (XGBoost) models — one for wind, and the other for solar — are trained on a **full calendar year** (`2025-01-01` → `2026-01-01`) so the `month` feature is observed across its entire 1–12 range. Training on only a few months previously caused the model to badly extrapolate for unseen months (verified: a July solar forecast produced ~150 kWh vs. ~810 kWh from a heuristic baseline for the same conditions — a sign of out-of-distribution extrapolation, not a real seasonal difference).
...
> If there's no `requirements.txt` yet, add one — at minimum you'll need `streamlit`, `fastapi`, `uvicorn`, `xgboost`, `pandas`, `numpy`, and whatever HTTP client `EnergyDataFetcher` uses to hit EirGrid.
...
Output: a dispatch schedule chart (charge/discharge over the horizon), cost/savings summary, and — for Stochastic MPC — a fan chart showing the range of forecasted scenarios considered.
